...
/// صفحه‌ی مستقل ویرایش یک «پیش‌فاکتور» موجود. کاملاً جدا از receipt_screen.dart
/// است تا منطق صدور فاکتور اصلی هرگز درگیر این قابلیت نشود.
///
/// شماره‌ی پیش‌فاکتور با ویرایش تغییر نمی‌کند و هیچ اثری روی موجودی کالا
/// یا stock_movements ندارد؛ اثر روی موجودی فقط هنگام «تبدیل به فاکتور
/// اصلی» (در جای دیگر) اعمال می‌شود.
class EditDraftInvoiceScreen extends StatefulWidget {
  final Invoice invoice;
  final List<InvoiceItem> items;
  final List<SideCost> sideCosts;
  const EditDraftInvoiceScreen({
    super.key,
  ...
  @override
  State<EditDraftInvoiceScreen> createState() => _EditDraftInvoiceScreenState();
}

class _EditDraftInvoiceScreenState extends State<EditDraftInvoiceScreen> {
  ...
  void initState() {
    ...
    _sideCosts = widget.sideCosts.map((c) => SideCostLine(title: c.title, amount: c.amount)).toList();
  ...
  Future<void> _pickCustomer() async {
    final selected = await showModalBottomSheet<Customer>(
      ...
      builder: (_) => const _DraftCustomerPickerSheet(),
    );
    if (selected != null) {
      setState(() {
  ...
  Future<void> _scanBarcodeForItem() async {
    if (_type.isServiceType) {
      _showMsg('اسکن بارکد فقط برای کالا در دسترس است');
      return;
    }
    ...
    final product = await _productRepo.getByBarcode(barcode);
    if (product != null) {
      ...
      _showMsg('${product.name} به فاکتور اضافه شد');
      return;
    }
    ...
    final createNew = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: const Text('کالای ناشناخته'),
        content: const Text('این کالا در بانک اطلاعاتی وجود ندارد. آیا می‌خواهید کالای جدید ثبت کنید؟'),
        actions: [
    ...
    if (createNew == true && mounted) {
      final newProduct = await Navigator.of(context).push<Product>(
        MaterialPageRoute(builder: (_) => ProductFormScreen(initialBarcode: barcode)),
      );
      if (newProduct != null) {
        setState(() => _lines.add(InvoiceCartLine(
              itemType: InvoiceItemType.product,
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (mounted) {
        _showMsg('پیش‌فاکتور به‌روزرسانی شد');
        Navigator.of(context).pop(true);
      }
    } catch (e) {
      _showMsg('خطا در ذخیره‌ی تغییرات: $e');
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('ویرایش پیش‌فاکتور ${PersianDateUtil.toPersianDigits(widget.invoice.invoiceNumber)}'),
      ),
      body: ListView(padding: const EdgeInsets.all(16), children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Chip(label: Text('پیش‌فاکتور ${PersianDateUtil.toPersianDigits(widget.invoice.invoiceNumber)}')),
        ]),
        const SizedBox(height: 16),
...
// ---------------- Customer picker (کپی مستقل، بدون وابستگی به receipt_screen.dart) ----------------

class _DraftCustomerPickerSheet extends StatefulWidget {
  const _DraftCustomerPickerSheet();
  @override
  State<_DraftCustomerPickerSheet> createState() => _DraftCustomerPickerSheetState();
}

class _DraftCustomerPickerSheetState extends State<_DraftCustomerPickerSheet> {
...
class _DraftServicePickerSheet extends StatefulWidget {
  ...
  @override
  State<_DraftServicePickerSheet> createState() => _DraftServicePickerSheetState();
}

class _DraftServicePickerSheetState extends State<_DraftServicePickerSheet> {
...
class _DraftProductPickerSheet extends StatefulWidget {
  ...
  @override
  State<_DraftProductPickerSheet> createState() => _DraftProductPickerSheetState();
}

class _DraftProductPickerSheetState extends State<_DraftProductPickerSheet> {
